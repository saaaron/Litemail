<?php 
	...
	// select posts 
	$select_important = "SELECT * FROM mails WHERE user_id = ? AND for_user_id = ? AND subject != '' AND important = 'yes' AND recycled = 'no' ORDER BY date_n_time DESC";
	$stmt = mysqli_prepare($db, $select_important);
	...
	$query = "SELECT * FROM mails WHERE user_id = ? AND for_user_id = ? AND subject != '' AND important = 'yes' AND recycled = 'no'";
