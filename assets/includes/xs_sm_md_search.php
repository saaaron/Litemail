<?php
  ...
  if (isset($_POST["query"])) {
    ...
    // mails
    $select_mails = "SELECT * FROM mails WHERE subject LIKE CONCAT('%', ?, '%') AND user_id = ? AND subject != '' AND recycled = 'no' LIMIT 6";
    ...
    echo 'Showing results for "<b>'.$keyword.'</b>" | <b>'.$total_mails.'</b> found';
  ...
  if ($keyword == true) {
    ...
    echo '
    <div class="text-center p-5 text-muted">
      <div class="bi-search-heart-fill" style="font-size: 20pt;"></div>
      <p>Search for emails using mail subject</p>
    </div>';
  }
?>